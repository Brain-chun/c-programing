...
class ListIter;
template < class ListElementType >  class List {
public:
	List() : head(0) { }
...
protected:  struct Node;
		 typedef Node* Link;
		 struct Node {
			 ListElementType elem;  Link next;
		 };
	Link head;
};
...
{
	if (iterPtr == 0)
		iterPtr = myList.head;  else
		iterPtr = iterPtr->next;  if (iterPtr)
		return iterPtr->elem;  else
		return myEndFlag;
}
...
List < ListElementType > ::insert(const ListElementType& elem)
{
	...
	assert(addedNode);  addedNode->elem = elem;  addedNode->next = head;  head = addedNode;
}

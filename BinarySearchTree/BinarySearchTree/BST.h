...
class BST : public BinaryTree < int > {
public:
	BST();
...
{
	if (nullTree) {
		nullTree = false;  leftTree = new BST;
		rightTree = new BST;  treeData = d;
	}
	else if (d == treeData)  treeData = d;
	else if (d < treeData)  leftTree->insert(d);
	else
		rightTree->insert(d);
}
template < class btElementType >  BinaryTree < btElementType >* BST < btElementType >

...
template < class tableKeyType, class tableDataType >  class Table
{
public:
...
private:
	...
	struct Slot;
	typedef Slot* Link;  struct Slot {
		tableKeyType key;  tableDataType data;  Link next;
	};
...
template < class tableKeyType, class tableDataType >  int
Table < tableKeyType, tableDataType >
::hash(const tableKeyType& key)
{
	return key % MAX_TABLE;
}
template < class tableKeyType, class tableDataType >  bool
Table < tableKeyType, tableDataType >
::search(Link& slotPointer, const tableKeyType& target)
{
	...
	for (; slotPointer; slotPointer = slotPointer->next)
		if (slotPointer->key == target)  return true;
	return false;
}
template < class tableKeyType, class tableDataType >  Table < tableKeyType, tableDataType >
::Table() // implementation of Table constructor
{
	int i;
	for (i = 0; i < MAX_TABLE; i++)  T[i] = 0;
}
template < class tableKeyType, class tableDataType >  void
Table < tableKeyType, tableDataType >
...
{
	int pos(hash(key));  Link sp(T[pos]);
	if (search(sp, key)) {
		data = sp->data;
		return true;
	}

...
void List::insert(const ListElementType& elem)
{
	...
	listArray[numberOfElements] = elem;  numberOfElements++;
}
...
bool List::next(ListElementType& elem)
{
	...
	// greater than or equal to zero  assert(currentPosition >= 0);
	if (currentPosition >= numberOfElements - 1)  return false;
	else {
		currentPosition++;
		elem = listArray[currentPosition];  return true;
	}
